#!/bin/bash
function menu {
    printf "\e[32m\e[1mMenu to start an instance:\n\e[0m"
...
while [ $OPTION != "3" ]; do
    if [ $OPTION == "1" ]; then
        IFS=$'\n'
        stoppedNAMES=`aws ec2 describe-instances --filters "Name=instance-state-name,Values=stopped" --query "Reservations[].Instances[].InstanceId" | cut -f2 -d \" | grep '[0-9]'`
        printf "\e[32m\e[1mThis are the instances that are stopped:\n\e[0m"
        for i in $stoppedNAMES; do
        ...
        read NAME
        printf "\e[32m\e[1mStarting the instance: $NAME...\n\e[0m"
        ...
        printf "\e[31m\e[1mInstance $NAME is started! \n\e[0m"
    fi
    ...
    if [ $OPTION == "2" ]; then
        ...
        read TAG_VALUE
        printf "\e[32m\e[1mStarting the instances with the tag value: $TAG_VALUE...\n\e[0m"
